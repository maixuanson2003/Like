...
import { createField, getFieldById, updateField } from 'services/danhmuc.service';
...
const Fields = () => {
    ...
    const handleUpdate = () => {
        ...
        updateField(dataEdit.id,data).then(res=>{
            handleClose();
            setRender(Date.now())
        }).catch(e=>{
          console.log("error")
    ...
    <div>
      ...
      <div className="p-8">
        <table className="w-full">
          <thead>
            <tr className="border border-[#eee] bg-[#F0F0F0]">
              ...
              <th className="border p-2">Mã ngành</th>
              <th className="border p-2">Ngành nghề</th>
            </tr>
          </thead>
          <tbody>
            <tr></tr>
        ...
        <div className='flex justify-end mt-10'>
            <Pagination currentPage={page} pageSize={size} totalRow={data.length} onPageChange={setPage} onSizeChange={setSize}/>
        </div>
      </div>
